...
// Promote turns a still-live ephemeral check result into a permanent,
// disk-backed report with its own ID. Gated by a dedicated per-IP limiter
// (10/hour) independent of every other rate-limiting layer in this
// codebase — see whatfind.md's rate-limiting section for why this needed
// its own dimension rather than reusing the subprocess semaphore or the
// general token bucket: it's a disk write, not a probe execution.
...
// ReportRead serves a promoted report by ID. Public and unauthenticated by
// design — it's a shareable link, same trust model as any other permalink.
